...
<font color="blue">生产者幂等性</font>：防止 消息重复写入 Kafka 分区，避免下游重复消费。
...
<font color="blue">消费者幂等性</font>

Kafka 本身是<font style="color:#DF2A3F;"> at-least-once </font>语义，因此消费者必须保证幂等。我采用数据库唯一索引保证幂等性。在生成订单时提前生成<font color="red">全局唯一 orderId</font>，并在订单表对 orderId 建立**唯一索引**。消费时直接 insert，如果成功说明首次消费；如果触发唯一索引异常，我会捕获异常并认为是幂等命中，直接 ack，从而保证重复消费不会影响系统状态。这种方式简单可靠，是生产环境中最常见的做法。

:::warning 注意
如果业务量极大，考虑使用 Redis 做第一层幂等过滤，数据库唯一索引做最终兜底，形成“双层幂等保护”。

Redis 是缓存层面的幂等，它只防止重复执行业务逻辑；数据库唯一索引 是最终的幂等保障，保证数据安全，即使 Redis 失效也不会重复操作。

:::

<font color="blue">Redis做第一层幂等过滤</font>：防止业务逻辑重复执行，尤其是在高并发下，避免数据库或核心业务被重复调用。

每个请求生成唯一标识（userId + skuId / requestId）
...
`acks` 参数：`acks=-1` 表示消息必须在被写入到主副本（Leader）以及所有副本（Follower）后，才返回响应。这个设置确保即使某个副本宕机，消息仍然能够被保留。
...
**手动提交 offset**：需要<font style="color:rgb(216,57,49);">禁用自动提交消息偏移量的机制</font>。因为在业务逻辑未完成时，可能会导致 offset 提交过早，导致消息丢失。手动提交 offset 可以确保只有在业务逻辑成功处理后，才更新消费进度，<font style="color:rgb(216,57,49);">避免漏消费</font>。
...
>
> 一个分区在<font style="color:rgb(216,57,49);">一个消费者组内</font>只能由一个消费者来消费，以确保每条消息只被消费一次。**一个消费者可以消费多个分区**，但 **每个分区只能由一个消费者消费**。
>
...
#### **（4）Redis库存扣减了，但是数据库的库存没有扣减、订单没有生成怎么办？**
...
⼀个典型的MQ系统，会将消息由⽣产者发送到MQ进⾏排队，然后根据⼀定的顺序交由消息的消费者进⾏处理。 QQ和微信就是典型的MQ。只不过他对接的使⽤对象是⼈，⽽Kafka需要对接的使⽤对象是应⽤程序。 
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://jiwang.cc.cd/PicGo/project/youxiang/2026-1774282620-74929e.png)
...
#### （4）Partition
<!-- 这是一张图片，ocr 内容为： -->
![](https://jiwang.cc.cd/PicGo/project/youxiang/2026-1774282620-db9eb0.png)
...
+ 提交失败：消费者可以选择重试或放弃。重试的话，可能会导致多次提交同一个 offset 值，但是不会影响正确性，因为 Kafka broker 会忽略重复的 offset 值。放弃的话，可能会导致下次启动时重新消费已经消费过的消息（系统视角）。

> 在 Kafka 中，**broker 和消费者都有各自的 offset，但语义不同**：broker 只关心“下一条要消费的消息编号”，即已经成功提交的 offset，表示下次消费者从哪里开始拉取消息；而消费者关注的是“业务系统是否已经处理过消息”，也就是内部逻辑的处理状态。由于提交 offset 可能失败，消费者可能已经处理了某条消息，但 broker 并不知道，这就产生了 **at-least-once** 语义。提交成功的 offset 会更新 broker，下次从该 offset 开始消费；提交失败的 offset 不会覆盖之前的成功 offset，消息本身不会丢失，但可能会被重复消费。因此，即便 broker offset 指向较新的消息，消费者处理失败的旧消息仍有可能被再次消费，消费者必须保证幂等性来避免重复执行。
...
:::danger 注意
Kafka 采用<font style="color:#DF2A3F;">拉模式</font>消费消息，即<font color="blue">消费者主动（系统视角）</font>通过 poll() 向 Broker 请求数据，而不是由 Broker 主动推送。这种设计的好处是消费者可以<font style="color:#DF2A3F;">根据自身处理能力控制拉取频率和批量大小，从而避免被高并发消息压垮</font>，同时提高批量处理效率，是 Kafka 高吞吐架构的重要基础。
